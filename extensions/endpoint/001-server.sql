...
create table endpoint.resource_function (
    ...
    path_pattern text not null, -- /blogs/{$1}/posts/{$2}.html -- the numbers correspond to the position of the argument passed to the specified function.  see site_settings.path_pattern_regex (really??)
...
/*
create function public.hi_mom(message text) returns text as $$
	select 'hi mom!  my message to you is: ' || message;
$$ language sql;
...
insert into endpoint.resource_function (
    ...
    '/hi_mom?message={$1}',
    (select id from endpoint.mimetype m where m.mimetype='text/plain')
);
